SELECT mm.dep , mm.mhead, mm.head, mm.scheme, mm.ga_ssa_status, mm.voted_charged_status, 
SUM(mm.bud), SUM(mm.expen) FROM 
(
...
(
	SELECT m.dep , m.mhead, m.head, m.scheme, m.ga_ssa_status, m.voted_charged_status,  SUM(amt) expen 
	FROM
	(
	...
	GROUP BY m.dep , m.mhead, m.head, m.scheme, m.ga_ssa_status, m.voted_charged_status
) expen
ON bud.dep = expen.dep
AND bud.mhead = expen.mhead
AND bud.scheme = expen.scheme
AND bud.head = expen.head
AND bud.ga_ssa_status = expen.ga_ssa_status
AND bud.voted_charged_status = expen.voted_charged_status
...
SELECT expen.dep, expen.mhead, expen.head, expen.scheme, expen.ga_ssa_status, expen.voted_charged_status, 
bud.bud, expen.expen FROM 
(
	SELECT 
	    hs.hierarchy_Name dep,
	    CONCAT( mh.head_code, '->', mh.head_name) mhead,
...
AND bud.head IS NULL AND bud.ga_ssa_status IS NULL AND bud.voted_charged_status IS NULL 

) mm 

GROUP BY mm.dep , mm.mhead, mm.head, mm.scheme, mm.ga_ssa_status, mm.voted_charged_status
