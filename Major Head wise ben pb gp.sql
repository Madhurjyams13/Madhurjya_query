SELECT m.mhead, 
SUM(m.incumbent_count), 
SUM(m.total_allowance), SUM(m.gp), SUM(m.pay), SUM(m.other)
FROM 
(
	SELECT st.incumbent_count, h.head, 
	CONCAT( mh.head_code, '->', mh.head_name) mhead, bd.bill_number, 
	...
	WHERE
	date(bd.token_date) BETWEEN '2024-04-01' AND '2024-05-06'
	...
	CONCAT( mh.head_code, '->', mh.head_name) , bd.bill_number, 
	bd.total_allowance, bd.id
)
m
GROUP BY m.mhead